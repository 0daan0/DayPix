...
#define SPI_SPEED 1000000  // 3 MHz SPI speed
#define LATCH_PIN 22  // GPIO pin 26 for latch signal
...
int wait_for_artsync = 0;  // Flag to control whether to wait for Art-Net Sync
int silent_mode = 0;  // Default is not silent mode

int spi_handle;  // Declare globally for access in all functions
...
void log_message(int level, const char *message, ...) {
    if (level <= log_level) {
        ...
        char formatted_message[512];  // Adjust the size as needed

        // Format the message with the arguments
        vsnprintf(formatted_message, sizeof(formatted_message), message, args);

        // Print the message with the appropriate log level
        switch (level) {
...
int initialize_system() {
    ...
    // Set LATCH_PIN as PI_OUTPUT
    gpioSetMode(LATCH_PIN, PI_OUTPUT);

    // Initialize SPI (Channel 0, 3 MHz)
    spi_handle = spiOpen(SPI_CHANNEL_0, SPI_SPEED, 0);
    if (spi_handle < 0) {
        log_message(LOG_LEVEL_ERROR, "SPI setup failed!");
        return -1;
    }
...
void write_to_spi(uint8_t *data, size_t length) {
    ...
    if (spiWrite(spi_handle, (char *)data, length) < 0) {
        log_message(LOG_LEVEL_ERROR, "Failed to write to SPI");
    } else {
...
void latch_led_PI_OUTPUT() {
    pthread_mutex_lock(&spi_mutex);  // Lock the mutex

    gpioWrite(LATCH_PIN, PI_HIGH);  // Set LATCH_PIN PI_HIGH
    gpioDelay(50);       // Small delay to register the latch
    gpioWrite(LATCH_PIN, PI_LOW);   // Set LATCH_PIN PI_LOW to finish the latching
...
void rainbow_chase() {
    uint8_t rainbow[MAX_DMX_SLOTS * 9];  // 3 bytes per LED (RGB)
...
void write_to_spi_with_gamma(uint8_t *data, size_t length) {
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        // Split the 16-bit value into two bytes (PI_HIGH byte and PI_LOW byte)
        spi_data[i * 2] = (corrected_value >> 8) & 0xFF;  // PI_HIGH byte
        spi_data[i * 2 + 1] = corrected_value & 0xFF;     // PI_LOW byte
    }
...
void write_frame_to_spi() {
    log_message(LOG_LEVEL_DEBUG, "Writing complete frame to SPI...");
    for (int i = 0; i < frame_buffer.expected_universes; i++) {
...
void handle_artnet_packet(uint8_t *buffer, size_t length) {
    ...
    log_message(LOG_LEVEL_DEBUG, "Handle packet");
    if (wait_for_artsync && op_code == 0x5200) {  // Art-Net Sync
        log_message(LOG_LEVEL_DEBUG, "Art-Net Sync message received.");
        if (is_frame_complete()) {
            write_frame_to_spi();
        } else {
            log_message(LOG_LEVEL_ERROR, "Incomplete frame, skipping PI_OUTPUT.");
        }
    ...
    {
        // Log the OpCode using your log_message function format
        char op_code_str[20];
        snprintf(op_code_str, sizeof(op_code_str), "0x%04X", op_code);
    
        // Concatenate "OpCode: " with the formatted OpCode string
        char log_msg[40];  // Increase size to fit both strings
        snprintf(log_msg, sizeof(log_msg), "OpCode: %s", op_code_str);
    
        log_message(LOG_LEVEL_DEBUG, log_msg);  // Log the OpCode as DEBUG level
    
    }

    // If it's a DMX Art-Net packet
    if (op_code == 0x5000) {  // DMX Art-Net packet
        ...
        // If the frame is complete and we are NOT waiting for Art-Net Sync, write it to SPI
        if (!wait_for_artsync && is_frame_complete()) {
            write_frame_to_spi();
        }
    } else {
        log_message(LOG_LEVEL_DEBUG, "Non-DMX Art-Net packet received.");
    }
}
...
void print_usage() {
    ...
    printf("  --silent            No startup LED test on output\n");
    printf("  --artsync           Enable wait for ArtSync messages\n"); 
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (strcmp(argv[i], "--artsync") == 0) {
            wait_for_artsync = 1;  // Set the flag to wait for Art-Net Sync
        } else if (strcmp(argv[i], "--silent") == 0) {
            silent_mode = 1;  // Set silent mode
        }
    }
    if (initialize_system() < 0) {
        log_message(LOG_LEVEL_ERROR, "System initialization failed.");
    ...
    // Print the configuration of command line parameters
    log_message(LOG_LEVEL_INFO, "Configuration:");
    log_message(LOG_LEVEL_INFO, "  Gamma Correction: %.2f", GAMMA);
    log_message(LOG_LEVEL_INFO, "  Universes to process: ");
    for (int i = 0; i < num_universes; i++) {
        printf("    Universe %d\n", universes[i]);
    }
    log_message(LOG_LEVEL_INFO, "  Reverse data: %s", reverse_data ? "Enabled" : "Disabled");
    log_message(LOG_LEVEL_INFO, "  Log Level: %d", log_level);
    log_message(LOG_LEVEL_INFO, "  Wait for Art-Net Sync: %s", wait_for_artsync ? "Enabled" : "Disabled");
    ...
    if (!silent_mode) {
       ...
       rainbow_chase();
       sleep(2);  // Wait for 2 seconds to show the rainbow chase effect
    }
