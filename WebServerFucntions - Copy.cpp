...
// Helper functions for webinterface data
String getUniverseOptions(int selected) {
    String options = "";
    for (int i = 0; i <= 15; ++i) {
...
String getDMXAddressOptions(int selected) {
    String options = "";
    for (int i = 0; i <= 512; ++i) {
        options += "<option value='" + String(i) + "'";
        if (i == selected) options += " selected";
...
String getNumPixelsOptions(int selected) {
    String options = "";
    for (int i = 1; i <= 170; ++i) {
        options += "<option value='" + String(i) + "'";
        if (i == selected) options += " selected";
...
void readEEPROMData() {
    storedDeviceName = getStoredString(DEV_NAME_EEPROM_ADDR);
...
void setupWebServer() {
  ...
  if (!SPIFFS.exists("/style.css")) {
   ...
   if (!SPIFFS.exists("/config.html")) {
    Serial.println("config html not found, will create new file");
    File htmlFile = SPIFFS.open("/config.html", FILE_WRITE, true);

     String html_page = config_html;
    if (htmlFile.print(html_page)) {
      Serial.println("Successfully wrote style to config.html");
      htmlFile.close();
    } else {
      Serial.println("ERROR: Writing to config.html failed");
    }
  }
 ...
 // Serve static files
  ...
  server.on("/getConfig", HTTP_GET, [](AsyncWebServerRequest *request) {
    String json = "{\"devicename\": \"" + storedDeviceName + "\", "
                  "\"ssid\": \"" + storedSSID  + "\", "
                  "\"password\": \"" + storedPassword  + "\", "
                  "\"b_failover\": \"" + (b_failover ? "checked" : "") + "\", "
                  "\"b_dhcp\": \"" + (b_dhcp ? "checked" : "") + "\", "
                  "\"ipAddress\": \"" + storedIPAddress    + "\", "
                  "\"subnetMask\": \"" + storedSubnetMask  + "\", "
                  "\"gateway\": \"" + storedGateway  + "\", "
                  "\"dnsServer\": \"" + storedDNSServer  + "\", "
                  "\"universe_start\": \"" + String(storedUniverseStrStart ) + "\", "
                  "\"universe_end\": \"" + String(storedUniverseStrEnd ) + "\", "
                  "\"DmxAddr\": \"" + String(storedDMXAddr ) + "\", "
                  "\"NrofLEDS\": \"" + String(storedNrOfLeds ) + "\", "
                  "\"b_16Bit\": \"" + (b_16Bit ? "checked" : "") + "\"}";
...
server.on("/save", HTTP_POST, [](AsyncWebServerRequest *request){
  ...
  if (request->hasParam("devicename", true)) devicename = request->getParam("devicename", true)->value();
  ...
  if (request->hasParam("subnetMask", true)) subnetMask = request->getParam("subnetMask", true)->value();
  if (request->hasParam("gateway", true)) gateway = request->getParam("gateway", true)->value();
  if (request->hasParam("dnsServer", true)) dnsServer = request->getParam("dnsServer", true)->value();
  if (request->hasParam("universe_start", true)) universe_start = request->getParam("universe_start", true)->value().toInt();
  if (request->hasParam("universe_end", true)) universe_end = request->getParam("universe_end", true)->value().toInt();
  if (request->hasParam("DmxAddr", true)) DmxAddr = request->getParam("DmxAddr", true)->value().toInt();
  if (request->hasParam("NrofLEDS", true)) NrofLEDS = request->getParam("NrofLEDS", true)->value().toInt();
  if (request->hasParam("b_16Bit", true)) b_16Bit = (request->getParam("b_16Bit", true)->value() == "on");
  if (request->hasParam("b_dhcp", true)) b_dhcp = (request->getParam("b_dhcp", true)->value() == "on");
  if (request->hasParam("b_failover", true)) b_failover = (request->getParam("b_failover", true)->value() == "on");

  // Convert integers to strings before saving
  storeString(SSID_EEPROM_ADDR, ssid);
  storeString(PASS_EEPROM_ADDR, password);
  storeString(UNIVERSE_EEPROM_ADDR, String(universe_start));  // Store as string
  storeString(DMX_ADDR_EEPROM_ADDR, String(DmxAddr));  // Store as string
  storeString(NRLEDS_EEPROM_ADDR, String(NrofLEDS));  // Store as string
  storeString(B_16BIT_EEPROM_ADDR, b_16Bit ? "on" : "off");  // Store as string
  storeString(B_FAILOVER_EEPROM_ADDR, b_failover ? "on" : "off");  // Store as string
  storeString(B_SILENT_EEPROM_ADDR, "off");  // Example value, update accordingly
  storeString(B_REVERSE_ARRAY_EEPROM_ADDR, "off");  // Example value, update accordingly
  storeString(DEV_NAME_EEPROM_ADDR, devicename);
  storeString(UNIVERSE_START_EEPROM_ADDR, String(universe_start));  // Store as string
  storeString(UNIVERSE_END_EEPROM_ADDR, String(universe_end));  // Store as string
  storeString(B_REVERSE_ARRAY_EEPROM_ADDR, "off");  // Example value, update accordingly

  storeString(B_DHCP_EEPROM_ADDR, b_dhcp ? "on" : "off");  // Store as string
  storeString(IP_ADDR_EEPROM_ADDR, ipAddress);
  storeString(IP_SUBNET_EEPROM_ADDR, subnetMask);
  storeString(IP_GATEWAY_EEPROM_ADDR, gateway);
  storeString(IP_DNS_EEPROM_ADDR, dnsServer);
  storeFloat(GAMMA_VALUE_EEPROM_ADDR, GAMMA_CORRECTION);  // Assuming GAMMA_CORRECTION is a float
